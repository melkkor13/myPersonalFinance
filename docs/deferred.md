...
| Item                                     | Note                                                                                                                                                                                                                   |
| ---------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **TLS**                                  | The scaffold runs on `localhost` over plain HTTP (assumption A4).                                                                                                                                                      |
| **Secret management**                    | `JWT_SECRET` comes from the environment. No KMS, no vault, no rotation.                                                                                                                                                |
| **Rate limiting on `/auth/*`**           | Nothing throttles login or refresh attempts.                                                                                                                                                                           |
| **Refresh-token storage in the browser** | Currently `localStorage` (`apps/web/src/api/tokens.ts`). Access token is in memory only. `localStorage` is readable by any XSS. **Replacing this is a prerequisite for any non-localhost deploy**, not a nice-to-have. |
| **CORS**                                 | Deliberately absent: Vite proxies `/api` in dev, so requests are same-origin. Adding CORS would mask a broken proxy.                                                                                                   |
| **Public registration**                  | Registration is a seed/CLI operation (assumption A5). There is no signup endpoint.                                                                                                                                     |
